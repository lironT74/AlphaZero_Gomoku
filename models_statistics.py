...
def collect_all_models_statistics(players_list, opponents, game_statistics_path,
                                  width=6, height=6, n=4,
                                  num_games=1000, sub_dir="statistics",
                                  **kwargs):
    ...
    for opponent_player in opponents:
        ...
        for cur_player in players_list:
            ...
            jobs.append((width, height, n, board_state, board_name, cur_player,
                         ...
                         p2, who_started_dict(board_name), num_games, sub_dir, game_statistics_path,  kwargs))
...
def calc_all_models_statistics(players_list, opponents, game_statistics_path, width=6, num_games=1000, sub_dir="statistics"):
    jobs = []
    for opponent_player in opponents:
        for board in TRUNCATED_BOARDS_DICT[width]:
            board_state, board_name, p1, p2, alternative_p1, alternative_p2 = board
            path = f"{game_statistics_path}{sub_dir}/vs {opponent_player}/{board_name}/"
...
def statistics_of_games_to_df(players_list, opponent_player, board, num_games, sub_dir, game_statistics_path):
    board_state, board_name, p1, p2, alternative_p1, alternative_p2 = board
    path = f"{game_statistics_path}{sub_dir}/vs {opponent_player}/{board_name}/"
...
def save_games_statistics(width, height, n, board_state, board_name, cur_player,
                          ...
                          correct_move_p2, start_player, num_games, sub_dir, game_statistics_path, kwargs):

    path = f"{game_statistics_path}{sub_dir}/vs {opponent_player.name}/{board_name}/"
    ...
    for i in range(num_games):
        ...
        all_games_history.append(
            (start_player, winner, game_length, shutter_sizes, real_last_move_shutter_sizes, game_history))
...
def get_statistics_from_saved_results(board_name, cur_player, opponent_player, num_games, sub_dir):
    ...
    games_history = pickle.load(
        open(f"{game_statistics_path}{sub_dir}/vs {opponent_player}/{board_name}/"
    ...
    for i in range(num_games):
        ...
        cur_player_range = [(index, shutter) for index, shutter in
                            zip(plays[0::2], shutter_sizes[cur_player_player_number]) if shutter != -1]
    ...
    print(
        f"{cur_player} vs {opponent_player} on {board_name}: {no_games}, {no_wins}, {no_losses}, {no_ties}  ({cur_time()})")

    average_game_length = np.average([x[1] for x in wins]) if len([x[1] for x in wins]) > 0 else -1
    average_game_length_wins = np.average([x[1] for x in wins if x[0] == cur_player]) if len(
        [x[1] for x in wins if x[0] == cur_player]) > 0 else -1
    average_game_length_losses = np.average([x[1] for x in wins if x[0] == opponent_player]) if len(
        [x[1] for x in wins if x[0] == opponent_player]) > 0 else -1

    avg_shutter_size = np.average(shutters_wins + shutters_losses + shutters_ties) if len(
        shutters_wins + shutters_losses + shutters_ties) > 0 else -1
    ...
    plays_with_shutter_fraction = len(
        shutters_wins + shutters_losses + shutters_ties) / total_plays if total_plays > 0 else 0
    ...
    CI_game_length_wins = bootstrap_mean([x[1] for x in wins if x[0] == cur_player]) if len(
        [x[1] for x in wins if x[0] == cur_player]) > 0 else [-1, -1]
    CI_game_length_losses = bootstrap_mean([x[1] for x in wins if x[0] == opponent_player]) if len(
        [x[1] for x in wins if x[0] == opponent_player]) > 0 else [-1, -1]

    CI_shutter_size = bootstrap_mean(shutters_wins + shutters_losses + shutters_ties) if len(
        shutters_wins + shutters_losses + shutters_ties) > 0 else [-1, -1]
    ...
    plays_with_shutter_sample_all_plays = np.concatenate(
        (np.ones(plays_with_shutter_all), np.zeros(plays_without_shutter_all)))
    ...
    plays_with_shutter_sample_wins_plays = np.concatenate(
        (np.ones(plays_with_shutter_wins), np.zeros(plays_without_shutter_wins)))
    ...
    plays_with_shutter_sample_losses_plays = np.concatenate(
        (np.ones(plays_with_shutter_losses), np.zeros(plays_without_shutter_losses)))
    ...
    if "random" in cur_player:
        ...
        avg_shutter_size_wins_real_last_turn = np.average(real_last_turn_shutters_cur_player_wins) if len(
            real_last_turn_shutters_cur_player_wins) > 0 else -1
        avg_shutter_size_losses_real_last_turn = np.average(real_last_turn_shutters_cur_player_losses) if len(
            real_last_turn_shutters_cur_player_losses) > 0 else -1
        ...
        plays_with_shutter_fraction_wins_real_last_turn = len(
            real_last_turn_shutters_cur_player_wins) / total_plays_wins if total_plays_wins > 0 else 0
        plays_with_shutter_fraction_losses_real_last_turn = len(
            real_last_turn_shutters_cur_player_losses) / total_plays_losses if total_plays_losses > 0 else 0
        ...
        CI_shutter_size_wins_real_last_turn = bootstrap_mean(real_last_turn_shutters_cur_player_wins) if len(
            real_last_turn_shutters_cur_player_wins) > 0 else [-1, -1]
        CI_shutter_size_losses_real_last_turn = bootstrap_mean(real_last_turn_shutters_cur_player_losses) if len(
            real_last_turn_shutters_cur_player_losses) > 0 else [-1, -1]
        ...
        plays_with_shutter_sample_all_plays_real_last_turn = np.concatenate(
            (np.ones(plays_with_shutter_all_real_last_turn), np.zeros(plays_without_shutter_all_real_last_turn)))
        ...
        plays_with_shutter_sample_wins_plays_real_last_turn = np.concatenate(
            (np.ones(plays_with_shutter_wins_real_last_turn), np.zeros(plays_without_shutter_wins_real_last_turn)))
        ...
        plays_with_shutter_sample_losses_plays_real_last_turn = np.concatenate(
            (np.ones(plays_with_shutter_losses_real_last_turn), np.zeros(plays_without_shutter_losses_real_last_turn)))
        CI_plays_with_shutter_losses_real_last_turn = bootstrap_mean(
            plays_with_shutter_sample_losses_plays_real_last_turn)
...
def the_twelve_6_X_6(num_games, limit_shutter, game_statistics_path):
    ...
    policy_v10_5000 = PolicyValueNet(6, 6, model_file=v10_5000[0], input_plains_num=v10_5000[2],
                                     shutter_threshold_availables=limit_shutter)
    player_v10_5000 = MCTSPlayer(policy_v10_5000.policy_value_fn, c_puct=5, n_playout=n_playout,
    ...
    # collect_all_models_statistics(players_list_6X6_vetran_5000, opponents_6X6, game_statistics_path, width=6, height=6, n=4, num_games=num_games, sub_dir=sub_dir)
    # collect_all_models_statistics(players_list_6X6, opponents_6X6, game_statistics_path, width=6, height=6, n=4, num_games=num_games, sub_dir=sub_dir)
    ...
    calc_all_models_statistics(p_names, o_names, game_statistics_path, width=6, num_games=num_games, sub_dir=sub_dir)
    plot_all_statistics_results(o_names, game_statistics_path, num_games=num_games, sub_dir=sub_dir, width=6)

    # save_states_from_history_empty_board(opponent_player_forcing_6X6, game_statistics_path, players_list_6X6, num_games, sub_dir=sub_dir, board_width=6)


def the_six_10X10(num_games, limit_shutter, game_statistics_path):
    ...
    # collect_all_models_statistics(players_list_10X10, opponents_10X10, game_statistics_path, width=10, height=10, n=5, num_games=num_games, sub_dir=sub_dir)
    ...
    calc_all_models_statistics(p_names, o_names, game_statistics_path, width=10, num_games=num_games, sub_dir=sub_dir)
    plot_all_statistics_results(o_names, game_statistics_path, num_games=num_games, sub_dir=sub_dir, width=10)

    # save_states_from_history_empty_board(opponent_player_forcing_10X10, game_statistics_path, p_names, num_games, sub_dir=sub_dir, board_width=10)



def run_statistics_pipeline(num_games, game_statistics_path):
    the_twelve_6_X_6(num_games, None, game_statistics_path)
    the_twelve_6_X_6(num_games, 0, game_statistics_path)

    the_six_10X10(num_games, None, game_statistics_path)
    the_six_10X10(num_games, 0, game_statistics_path)
    the_six_10X10(num_games, 1, game_statistics_path)
    the_six_10X10(num_games, 2, game_statistics_path)


def get_opponents_names(board_size):

    if board_size == 6:
        opponents = [opponent_player_forcing_6X6, opponent_player_mcts_500_6X6,
                         opponent_player_mcts_1000_6X6, opponent_player_v9_5000_6X6,
                         opponent_player_v10_5000_6X6]

    else:
        opponents = [opponent_player_forcing_10X10,
                           opponent_player_mcts_500_10X10, opponent_player_3_mcts_1000_10X10,
                           opponent_player_v_01_5000_10X10, opponent_player_v_02_5000_10X10]
...
def shutter_comparison_winratio_plots():

    plot_shutter_comparison_winratio_plots(game_statistics_path, board_size=6,  num_games=1000, limits_shutter=[None, 0])
    plot_shutter_comparison_winratio_plots(game_statistics_path, board_size=10, num_games=1000, limits_shutter=[None, 0])
    plot_shutter_comparison_winratio_plots(game_statistics_path, board_size=10, num_games=1000, limits_shutter=[None, 1, 0])
    plot_shutter_comparison_winratio_plots(game_statistics_path, board_size=10, num_games=1000, limits_shutter=[None, 2, 1, 0])
...
def plot_all_statistics_results(opponents, game_statistics_path, num_games, sub_dir, width):
    jobs = []
    for opponent_player in opponents:
        for board in TRUNCATED_BOARDS_DICT[width]:
            board_name = board[1]
            jobs.append((num_games, opponent_player, board_name, sub_dir, width, game_statistics_path))
    ...
    for board in BOARDS:
        board_name = board[1]
        jobs_collage.append((board_name, opponents, sub_dir, width, game_statistics_path))
...
def create_statistics_graphics(num_games, opponent_name, board_name, sub_dir, board_width, game_statistics_path):
    path = f"{game_statistics_path}{sub_dir}/vs {opponent_name}/{board_name}/"
    ...
    for variation in all_models_variations[board_width]:
        ...
        save_plays_with_shutter_results(board_name, path, num_games, width, height, font_size, opponent_name, data,
                                        group_by=group_by)
        save_shutter_size(board_name, path, num_games, width, height, font_size, opponent_name, data, group_by=group_by)
        save_save_game_len(board_name, path, num_games, width, height, font_size, opponent_name, data,
                           group_by=group_by)
        save_game_results(board_name, path, num_games, width, height, font_size, opponent_name, data, group_by=group_by)
        save_win_ratio_no_ties(board_name, path, num_games, width, height, font_size, opponent_name, data,
                               group_by=group_by, is_interchangeable_color=is_interchangeable_color)


def save_plays_with_shutter_results(board_name, path, num_games, fig_width, height, font_size, opponent_name, data,
                                    group_by=None):
    ...
    ax.bar(ind - width, data["fraction of plays with shutter"][:-1], width=width,
           label="fraction of plays with shutter", color="blue")
    ax.bar(ind, data["fraction of plays with shutter (wins)"][:-1], width=width,
           label="fraction of plays with shutter - wins", color="green")
    ax.bar(ind + width, data["fraction of plays with shutter (losses)"][:-1], width=width,
           label="fraction of plays with shutter - losses", color="red")
    ...
    ax.bar(len(data.index) - 0.5 - 2 * width, data["fraction of plays with shutter (real last turn)"][-1],
           label="real last turn", width=width, color="cornflowerblue")

    ax.bar(len(data.index) - 0.5 - 0.5 * width, data["fraction of plays with shutter (wins)"][-1], width=width,
           color="green")
    ax.bar(len(data.index) - 0.5 + 0.5 * width, data["fraction of plays with shutter (real last turn - wins)"][-1],
           label="real last turn", width=width, color="greenyellow")

    ax.bar(len(data.index) - 0.5 + 2 * width, data["fraction of plays with shutter (losses)"][-1], width=width,
           color="red")
    ax.bar(len(data.index) - 0.5 + 3 * width, data["fraction of plays with shutter (real last turn - losses)"][-1],
           label="real last turn", width=width, color="lightcoral")

    ax.plot((len(data.index) - 0.5 - 3 * width, len(data.index) - 0.5 - 3 * width),
            npstr2tuple(data["CI_plays_with_shutter_all"][-1]), 'r_-', color='black', linewidth=4, mew=4, ms=20)
    ax.plot((len(data.index) - 0.5 - 2 * width, len(data.index) - 0.5 - 2 * width),
            npstr2tuple(data["CI_plays_with_shutter_all_real_last_turn"][-1]), 'r_-', color='black', linewidth=4, mew=4,
            ms=20)

    ax.plot((len(data.index) - 0.5 - 0.5 * width, len(data.index) - 0.5 - 0.5 * width),
            npstr2tuple(data["CI_plays_with_shutter_wins"][-1]), 'r_-', color='black', linewidth=4, mew=4, ms=20)
    ax.plot((len(data.index) - 0.5 + 0.5 * width, len(data.index) - 0.5 + 0.5 * width),
            npstr2tuple(data["CI_plays_with_shutter_wins_real_last_turn"][-1]), 'r_-', color='black', linewidth=4,
            mew=4, ms=20)

    ax.plot((len(data.index) - 0.5 + 2 * width, len(data.index) - 0.5 + 2 * width),
            npstr2tuple(data["CI_plays_with_shutter_losses"][-1]), 'r_-', color='black', linewidth=4, mew=4, ms=20)
    ax.plot((len(data.index) - 0.5 + 3 * width, len(data.index) - 0.5 + 3 * width),
            npstr2tuple(data["CI_plays_with_shutter_losses_real_last_turn"][-1]), 'r_-', color='black', linewidth=4,
            mew=4, ms=20)
...
def save_shutter_size(board_name, path, num_games, fig_width, height, font_size, opponent_name, data, group_by=None):
    ...
    ax.bar(ind + width, data["avg shutter size (losses)"][:-1], width=width, label="average shutter size - losses",
           color="red")
    ...
    ax.bar(len(data.index) - 0.5 - 2 * width, data["avg shutter size (real last turn)"][-1], label="real last turn",
           width=width, color="cornflowerblue")

    ax.bar(len(data.index) - 0.5 - 0.5 * width, data["avg shutter size (wins)"][-1], width=width, color="green")
    ax.bar(len(data.index) - 0.5 + 0.5 * width, data["avg shutter size (real last turn - wins)"][-1],
           label="real last turn", width=width, color="greenyellow")

    ax.bar(len(data.index) - 0.5 + 2 * width, data["avg shutter size (losses)"][-1], width=width, color="red")
    ax.bar(len(data.index) - 0.5 + 3 * width, data["avg shutter size (real last turn - losses)"][-1],
           label="real last turn", width=width, color="lightcoral")

    ax.plot((len(data.index) - 0.5 - 3 * width, len(data.index) - 0.5 - 3 * width),
            npstr2tuple(data["CI_shutter_size"][-1]), 'r_-', color='black', linewidth=4, mew=4, ms=20)
    ax.plot((len(data.index) - 0.5 - 2 * width, len(data.index) - 0.5 - 2 * width),
            npstr2tuple(data["CI_shutter_size_real_last_turn"][-1]), 'r_-', color='black', linewidth=4, mew=4, ms=20)

    ax.plot((len(data.index) - 0.5 - 0.5 * width, len(data.index) - 0.5 - 0.5 * width),
            npstr2tuple(data["CI_shutter_size_wins"][-1]), 'r_-', color='black', linewidth=4, mew=4, ms=20)
    ax.plot((len(data.index) - 0.5 + 0.5 * width, len(data.index) - 0.5 + 0.5 * width),
            npstr2tuple(data["CI_shutter_size_wins_real_last_turn"][-1]), 'r_-', color='black', linewidth=2, mew=2,
            ms=20)

    ax.plot((len(data.index) - 0.5 + 2 * width, len(data.index) - 0.5 + 2 * width),
            npstr2tuple(data["CI_shutter_size_losses"][-1]), 'r_-', color='black', linewidth=4, mew=4, ms=20)
    ax.plot((len(data.index) - 0.5 + 3 * width, len(data.index) - 0.5 + 3 * width),
            npstr2tuple(data["CI_shutter_size_losses_real_last_turn"][-1]), 'r_-', color='black', linewidth=2, mew=2,
            ms=20)
...
def save_win_ratio_no_ties(board_name, path, num_games, fig_width, height, font_size, opponent_name, data,
                           group_by=None, is_interchangeable_color=False):
    mpl.rcParams.update({'font.size': font_size})
...
def save_game_results(board_name, path, num_games, fig_width, height, font_size, opponent_name, data, group_by=None):
    ...
    ax.bar(ind, data["no. ties"], width=width, label="ties", color="yellow",
           bottom=data["no. wins"] + data["no. losses"])
...
def call_collage_statistics_results(board_name, opponents, sub_dir, board_width, game_statistics_path):
    opponents_names = [opp for opp in opponents]

    path_collage = f"{game_statistics_path}{sub_dir}/{board_name} summery/"
    ...
    for variation in all_models_variations[board_width]:
        ...
        listofimages1 = [
            f"{game_statistics_path}{sub_dir}/vs {opponent_name}/{board_name}/Fraction of plays with shutter results {group_by}.png"
            for opponent_name in opponents_names]

        listofimages2 = [
            f"{game_statistics_path}{sub_dir}/vs {opponent_name}/{board_name}/Games lengths results {group_by}.png"
            for opponent_name in opponents_names]

        listofimages3 = [
            f"{game_statistics_path}{sub_dir}/vs {opponent_name}/{board_name}/Games results {group_by}.png"
            for opponent_name in opponents_names]

        listofimages4 = [
            f"{game_statistics_path}{sub_dir}/vs {opponent_name}/{board_name}/Shutter sizes results {group_by}.png"
            for opponent_name in opponents_names]

        listofimages5 = [
            f"{game_statistics_path}{sub_dir}/vs {opponent_name}/{board_name}/Win ratio ignoring ties {group_by}.png"
            for opponent_name in opponents_names]
...
def save_states_from_history_empty_board(opponent, game_statistics_path, players_list, num_games, sub_dir, board_width):
    path = f"{game_statistics_path}{sub_dir}/vs {opponent.name}/empty board/"
    start_player = 1
...
def win_ratio_comparison_plot(board_size, num_games, fig_width, height, font_size, opponent_name, limits_shutter,
                              board_name, variation, game_statistics_path):
    ...
    if len(data_dict) == 2:
        ax.bar(ind - width / 4, data_dict[0]["no. wins"] * (1 / (num_games - data_dict[0]["no. ties"])),
               width=width_ratio * width, color='grey', alpha=0.5, label=f'No playtime limitations')
        ax.bar(ind + width / 4, data_dict[1]["no. wins"] * (1 / (num_games - data_dict[1]["no. ties"])),
               width=width_ratio * width, color='blue', alpha=0.5,
               label=f'Playtime: limit to shutter {limits_shutter[1]}')

        for index, ci in zip(ind, data_dict[0]["CI_wins_losses"]):
            ax.plot((index - width / 4, index - width / 4), npstr2tuple(ci), 'r_-', color='black', linewidth=4, mew=4,
                    ms=40)
    ...
    if len(data_dict) == 3:
        ax.bar(ind - width * (1 / 2), data_dict[0]["no. wins"] * (1 / (num_games - data_dict[0]["no. ties"])),
               width=width_ratio * width, color='grey', alpha=0.5, label=f'No playtime limitations')
        ax.bar(ind, data_dict[1]["no. wins"] * (1 / (num_games - data_dict[1]["no. ties"])), width=width_ratio * width,
               color='red', alpha=0.5, label=f'Playtime: limit to shutter {limits_shutter[1]}')
        ax.bar(ind + width * (1 / 2), data_dict[2]["no. wins"] * (1 / (num_games - data_dict[2]["no. ties"])),
               width=width_ratio * width, color='blue', alpha=0.5,
               label=f'Playtime: limit to shutter {limits_shutter[2]}')

        for index, ci in zip(ind, data_dict[0]["CI_wins_losses"]):
            ax.plot((index - width * (1 / 2), index - width * (1 / 2)), npstr2tuple(ci), 'r_-', color='black',
                    linewidth=4, mew=4, ms=30)
    ...
    if len(data_dict) == 4:
        ax.bar(ind - width * skip, data_dict[0]["no. wins"] * (1 / (num_games - data_dict[0]["no. ties"])),
               width=width_ratio * width, color='grey', alpha=0.5, label=f'No playtime limitations')
        ax.bar(ind - width * (1 / 3) * skip, data_dict[1]["no. wins"] * (1 / (num_games - data_dict[1]["no. ties"])),
               width=width_ratio * width, color='green', alpha=0.5,
               label=f'Playtime: limit to shutter {limits_shutter[1]}')
        ax.bar(ind + width * (1 / 3) * skip, data_dict[2]["no. wins"] * (1 / (num_games - data_dict[2]["no. ties"])),
               width=width_ratio * width, color='red', alpha=0.5,
               label=f'Playtime: limit to shutter {limits_shutter[2]}')
        ax.bar(ind + width * skip, data_dict[3]["no. wins"] * (1 / (num_games - data_dict[3]["no. ties"])),
               width=width_ratio * width, color='blue', alpha=0.5,
               label=f'Playtime: limit to shutter {limits_shutter[3]}')
    ...
    path = f"{game_statistics_path}plots_and_summery/{board_size}X{board_size}_shutter_{limit_shtter_str}/vs {opponent_name}/{board_name}/"
...
def win_ratio_comparison_plot_collage(board_name, opponents, game_statistics_path, board_size, variation, limits_shutter=[None, 0]):
    opponents_names = [opp for opp in opponents]

    limit_shtter_str = '_'.join([str(lim) for lim in limits_shutter])

    path_collage = f"{game_statistics_path}plots_and_summery/{board_size}X{board_size}_shutter_{limit_shtter_str}/{board_name} summery/"
    ...
    if variation:
        listofimages = [
            f"{game_statistics_path}plots_and_summery/{board_size}X{board_size}_shutter_{limit_shtter_str}/vs {opponent_name}" \
            f"/{board_name}/Win ratio ignoring ties comparison {variation[1]}.png"
            for opponent_name in opponents_names]

        create_collages_boards(listofimages, f"Win ratio ignoring ties shutter size comparison {variation[1]}",
                               path_collage)
...
def plot_shutter_comparison_winratio_plots(game_statistics_path, board_size=6, num_games=1000, limits_shutter=[None, 0]):
    fig_width = 45
    height = 10
    font_size = 27
    ...
    opponents = get_opponents_names(board_size)
    ...
    for variation in variations[board_size]:
        ...
        for opponent_player in opponents:
            ...
            for board in TRUNCATED_BOARDS_DICT[board_size]:
                board_name = board[1]
                jobs.append((board_size, num_games, fig_width, height, font_size, opponent_player, limits_shutter,
                             board_name, variation, game_statistics_path))
        ...
        for board in BOARDS:
            board_name = board[1]
            jobs_collage.append((board_name, opponents, game_statistics_path, board_size, variation, limits_shutter))
