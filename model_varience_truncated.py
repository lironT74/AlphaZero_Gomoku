...
def model_var_emd_board(model_name,
    ...
    for i in range(model_check_freq, max_model_iter + model_check_freq, model_check_freq):
        ...
        var_list.append(np.var(emd_list))

    save_fig_var(var_list, models_num, model_list, model_name, board_name)
...
def save_fig_var(var_list, models_num, model_list, model_name, board_name):
    ...
    plt.savefig(f"{path}{model_name}_{board_name}_variance.png")

    plt.close('all')



def model_avg_emd_board(model_name,
                        input_plains_num,
                        game_board,
                        curr_player=1,
                        max_model_iter = 5000,
    ...
    for i in range(model_check_freq, max_model_iter + model_check_freq, model_check_freq):
        ...
        avg_list.append(np.average(emd_list))

    save_fig_avg(avg_list, models_num, model_list, model_name, board_name)


def save_fig_avg(avg_list, models_num, model_list, model_name, board_name):
    ...
    ax.plot(range(models_num), avg_list, color="blue", linewidth=linewidth)
    ...
    ax.tick_params(axis='both', which='major', labelsize=fontsize)
    ax.set_xlabel("sub model no.", fontsize=fontsize)
    ax.set_title(f"Average distance between the policy with the correct last move to rest\n of the policies with all the other possible last moves on {board_name}",
                 fontdict={'fontsize': fontsize + 15})
    ...
    plt.savefig(f"{path}{model_name}_{board_name}_average.png")
...
if __name__ == '__main__':
    ...
    for board in PAPER_TRUNCATED_BOARDS:
        model_avg_emd_board('pt_6_6_4_p4_v10',
                            4,
                            board,
                            curr_player=1,
                            max_model_iter=5000,
